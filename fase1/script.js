...
function checarFimDoJogo() {
    ...
    if (botoes.length === 0 || Array.from(botoes).every(botao => botao.disabled)) {
        document.getElementById('resultado').textContent = `Você encontrou ${acertos} biomas corretos!`;
...
function reiniciarJogo() {
    iniciarJogo();
    document.getElementById('reiniciar').style.display = 'none'; // Esconder botão de reiniciar
}
